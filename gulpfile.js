...
function html() {
  return gulp.src(['src/**/*.html', '!src/partsHTML/*.html'])
    .pipe(include({
      prefix:'@@'
    }))
...
function img() {
  return gulp.src('src/**/*.{jpg,jpeg,png,svg,gif,ico,webp,mp4,ogv,webm}')
    .pipe(gulp.dest('dist/'))
}
exports.img = img
...
exports.production = gulp.series(clear,allcss,js_prod,jsCopy,fileCopy,img,delSvgIconSrc,delSvgIconDist,createSprite,convertFont,html_for_prod,replacehtml)

//разработка
exports.serve = gulp.series(less,js_prod,jsCopy,fileCopy,img,delSvgIconSrc,delSvgIconDist,createSprite,convertFont,html,serve)
